...
class Ellipse extends Drawable {
    ...
    draw(selected){
        ctx.beginPath();
        ctx.ellipse(Math.abs(this.x), Math.abs(this.y), Math.abs(this.width), Math.abs(this.height), Math.PI * 2, 0, Math.PI * 2);
        ctx.closePath();
        ctx.fillStyle = this.color;
        if (selected){
            ctx.lineWidth = 2;
    ...
    in(x,y){
        return this.distanceTo(x,y) < Math.abs(this.width) && 
               this.distanceTo(x,y) < Math.abs(this.height);
    }
